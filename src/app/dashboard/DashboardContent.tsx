...
function JokesTable({ stats }: { stats: Record<string, JokeAnalytics> }) {
  ...
  const filtered = useMemo(() => {
    const list = langFilter === "all" ? jokes : jokes.filter((j) => j.language === langFilter);
    return list
      .map((j) => ({
        joke: j,
        stats: stats[j.id] ?? {
...
function AlgorithmPanel({ weights, updateWeights }: {
  ...
  const sliders: { key: keyof typeof weights; label: string; emoji: string }[] = [
    { key: "likeWeight", label: "Like Weight", emoji: "❤️" },
    { key: "shareWeight", label: "Share Weight", emoji: "📤" },
    { key: "timeOnCardWeight", label: "Time on Card", emoji: "⏱️" },
    { key: "correctAnswerWeight", label: "Correct Answer", emoji: "✅" },
    { key: "recencyWeight", label: "Recency", emoji: "🕐" },
    { key: "diversityWeight", label: "Diversity", emoji: "🌈" },
  ];
    ...
    <div className="space-y-6">
      ...
      <div className="bg-surface/30 border border-white/5 rounded-2xl p-4">
        ...
        <div className="text-xs text-white/50 space-y-1.5">
          ...
          <p>
            <strong className="text-white/70">Engagement Score</strong> = (LikeRate × LikeWeight) + (ShareRate × ShareWeight) + (TimeScore × TimeWeight) + (CorrectRate × CorrectWeight) + (RecencyBonus × RecencyWeight) + (DiversityBonus × DiversityWeight)
          </p>
