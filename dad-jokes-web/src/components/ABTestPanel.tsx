...
import type { DadJoke, JokeAnalytics, ABTest, ABVariant } from "@/lib/types";
...
function WinnerSuggestion({ test, jokeStats, allJokes }: { test: ABTest; jokeStats: Record<string, JokeAnalytics>; allJokes: DadJoke[] }) {
  ...
  const topJokes = [...winner.variant.jokeIds]
    .map((id) => ({ id, score: jokeStats[id]?.engagementScore ?? 0, joke: allJokes.find((j) => j.id === id) }))
    .filter((x) => x.joke)
...
function TestCard({ test, jokeStats, allJokes }: { test: ABTest; jokeStats: Record<string, JokeAnalytics>; allJokes: DadJoke[] }) {
  ...
  return (
    <div className="bg-surface/30 border border-white/5 rounded-2xl overflow-hidden">
      <div className="p-4 space-y-4">
        ...
        {/* Winner suggestion */}
        <WinnerSuggestion test={test} jokeStats={jokeStats} allJokes={allJokes} />
      </div>
    </div>
  );
}
...
export default function ABTestPanel({ jokeStats, allJokes }: { jokeStats: Record<string, JokeAnalytics>; allJokes: DadJoke[] }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-4">
        {tests.map((test) => (
          <TestCard key={test.id} test={test} jokeStats={jokeStats} allJokes={allJokes} />
        ))}
        {tests.length === 0 && (
