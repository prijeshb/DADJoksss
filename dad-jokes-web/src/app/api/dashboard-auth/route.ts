...
export async function POST(req: NextRequest) {
  ...
  // Fail-closed: deny access if PIN is not configured or too short
  if (!configuredPin || configuredPin.length < MIN_PIN_LENGTH) {
  ...
  res.cookies.set("dash_session", expectedToken, {
    httpOnly: true,
    secure: true, // always; also enforces HTTPS in local dev (use https:// locally)
    sameSite: "strict",
    path: "/",
