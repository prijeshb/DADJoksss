...
describe("POST /api/dashboard-auth", () => {
  describe("when DASHBOARD_PIN is not set (fail-closed)", () => {
    it("returns 503 when DASHBOARD_PIN is empty", async () => {
    ...
    it("returns 503 when DASHBOARD_PIN is too short (< 8 chars)", async () => {
      vi.stubEnv("DASHBOARD_PIN", "short");
      const { POST: handler } = await import("./route");
      const res = await handler(makeRequest({ pin: "short" }) as never);
      expect(res.status).toBe(503);
      const body = await res.json();
      expect(body.ok).toBe(false);
    });

    it("accepts DASHBOARD_PIN of exactly 8 characters", async () => {
      vi.stubEnv("DASHBOARD_PIN", "exactly8");
      const { POST: handler } = await import("./route");
      const res = await handler(makeRequest({ pin: "exactly8" }) as never);
  ...
  describe("when DASHBOARD_PIN is configured", () => {
    ...
    it("does not set a cookie on a failed auth attempt", async () => {
      const { POST: handler } = await import("./route");
      const res = await handler(makeRequest({ pin: "wrongpin" }, "10.0.1.1") as never);
      expect(res.status).toBe(401);
      expect(res.headers.get("set-cookie")).toBeNull();
    });

    it("rate limits on the 6th attempt even when the 6th uses the correct PIN", async () => {
      const { POST: handler } = await import("./route");
      const ip = "10.1.0.1";
      for (let i = 0; i < 5; i++) {
        await handler(makeRequest({ pin: "wrong" }, ip) as never);
    ...
    it("uses only the first IP from a comma-separated x-forwarded-for header", async () => {
      const { POST: handler } = await import("./route");
      const ip = "10.1.0.2";

      // Exhaust rate limit under the real client IP (first in the chain)
      for (let i = 0; i < 5; i++) {
        await handler(makeRequest({ pin: "wrong" }, `${ip}, 192.168.1.1`) as never);
      }

      // A 6th request with the same first IP (different proxy) must still be blocked
      const res = await handler(makeRequest({ pin: PIN }, `${ip}, 10.99.0.5`) as never);
      expect(res.status).toBe(429);
    });

    it("treats pin as non-string (number) as an invalid submission returning 401", async () => {
      const { POST: handler } = await import("./route");
      // Send pin as number — body.pin is not a string; route reads body.pin ?? "" = undefined ?? "" = ""
      const res = await handler(makeRequest({ pin: 1234 } as unknown as { pin: string }, "10.1.0.3") as never);
      expect(res.status).toBe(401);
    });

    it("treats null pin as an invalid submission returning 401", async () => {
      const { POST: handler } = await import("./route");
      const res = await handler(makeRequest({ pin: null } as unknown as { pin: string }, "10.1.0.4") as never);
      expect(res.status).toBe(401);
    });

    it("cookie has maxAge of 8 hours (28800 seconds)", async () => {
      const { POST: handler } = await import("./route");
      const res = await handler(makeRequest({ pin: PIN }, "10.1.0.5") as never);
      expect(res.status).toBe(200);
      const setCookie = res.headers.get("set-cookie") ?? "";
